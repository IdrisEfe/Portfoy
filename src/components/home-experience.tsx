...
import { contentCopy } from "@/lib/content";
...
export function HomeExperience({ repositories }: { repositories: Repository[] }) {
  ...
  const text = contentCopy(content)[locale];
  const socialsById = Object.fromEntries(content.socials.map((social) => [social.id, social]));
  ...
  return <>
    <section className="hero section-grid">
      ...
      <div className="hero-mask"><DigitalMask mode={content.avatar.mode} /><div className="mask-label top">IDENTITY / {content.avatar.mode === "face" ? "3D" : "LIVE"}</div><div className="mask-label bottom">MOVE TO REORIENT · TAP TO PULSE</div></div>
    ...
    <section className="split-feature content-section">
      ...
      <div className="feature-list">
        {content.showcase.filter((item) => item.visible).map((item, index) => <Link href={item.href} key={item.id}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{item.title}</strong><p>{item.type === "project" ? "A selected project signal." : "A different way into the work."}</p></div><ArrowRight /></Link>)}
      </div>
    </section>
    <PublicMetrics />
