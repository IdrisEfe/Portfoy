...
export async function GET(request: Request) {
  ...
  if (!code || !state || state !== expectedState) return NextResponse.redirect(new URL("/admin?error=invalid_state", siteUrl));
  ...
  if (!tokenData.access_token) return NextResponse.redirect(new URL("/admin?error=oauth_failed", siteUrl));
  ...
  if (!user.id || !user.login || !((allowedId && user.id === allowedId) || (allowedLogin && user.login.toLowerCase() === allowedLogin))) return NextResponse.redirect(new URL("/admin?error=unauthorized", siteUrl));
  const session = sealSession({ id: user.id, login: user.login, accessToken: tokenData.access_token, expiresAt: Date.now() + 8 * 60 * 60 * 1000 });
  const response = NextResponse.redirect(new URL("/admin", siteUrl)); response.cookies.set(adminCookie.name, session, adminCookie.options); return response;
}
